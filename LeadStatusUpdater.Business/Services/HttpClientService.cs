    ...
    public async Task<T> Get<T>(string urlForRequest, CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessful)
            throw new HttpRequestException(
                $"Request failed with status code {response.StatusCode} and message: {response.ErrorMessage}, Content: {response.Content}");
        ...
        try
        {
            ...
            if (result == null) throw new HttpRequestException("Deserialized response is null");
